...
namespace chfs
{
  ...
  auto FileOperation::alloc_inode(InodeType type) -> ChfsResult<inode_id_t>
  {
    ...
    // std::cout << "in FileOperation::alloc_inode inode_id: " << inode_id << std::endl;
      ...
      -> ChfsNullResult
  {
    ...
    block_id_t *block_array = reinterpret_cast<block_id_t *>(indirect_block.data());
    ...
    if (new_block_num > old_block_num)
    {
      ...
      for (usize idx = old_block_num; idx < new_block_num; ++idx)
      {
        ...
        {
          // std::cout << "direct idx " << idx << std::endl;
          // std::cout << "direct block_id " << block_id << std::endl;
          inode_p->set_block_direct(idx, block_id);
        }
        else
        {
          // std::cout << "indirect idx " << idx << std::endl;
          // std::cout << "indirect block_id " << block_id << std::endl;
          ...
          // indirect_block.push_back(u8_id);
          block_array[idx - inlined_blocks_num] = block_id;
          fix = true;
          // std::cout << " idx - inlined_blocks_num   " << idx - inlined_blocks_num << std::endl;
        }
    ...
    else
    {
      ...
      for (usize idx = new_block_num; idx < old_block_num; ++idx)
      {
        ...
        else
        {
          ...
          auto indirect_block_id = res.unwrap();
          ...
          this->block_allocator_->deallocate(indirect_block_p[idx - inlined_blocks_num]);
          // std::cout << "deallocate indirect_block ID : " << indirect_block_p[idx - inlined_blocks_num] << std::endl;
...

    {
      ...
      while (write_sz < content.size())
      {
        ...
        else
        {
          ...
          // UNIMPLEMENTED();
          // std::cout << "block_idx - inlined_blocks_num" << block_idx - inlined_blocks_num << std::endl;
          if (block_array[block_idx - inlined_blocks_num] != KInvalidBlockID)
          {
            block_id = block_array[block_idx - inlined_blocks_num];
            // std::cout << "write content1111 indirect block_id: " << block_id << std::endl;
          }
          else
          {
            std::vector<u8> tmp(0);
            ...
            auto read_res = this->block_manager_->read_block(indirect_block_id, tmp.data());
            if (read_res.is_err())
            {
              error_code = read_res.unwrap_error();
              goto err_ret;
            }
            auto indirect_block_p = reinterpret_cast<block_id_t *>(tmp.data());
            block_id = indirect_block_p[block_idx - inlined_blocks_num];
            block_array[block_idx - inlined_blocks_num] = block_id;
            // std::cout << "block_idx - inlined_blocks_num: " << block_idx - inlined_blocks_num << std::endl;
            // std::cout << "write content2222 indirect block_id: " << block_id << std::endl;
          }
        }
  ...
  auto FileOperation::read_file(inode_id_t id) -> ChfsResult<std::vector<u8> >
  {
    ...
    while (read_sz < file_sz)
    {
      ...
      else
      {
        ...
        // UNIMPLEMENTED();
        auto indirect_block_id = inode_p->get_indirect_block_id();
        // std::cout << "indirect block id: " << indirect_block_id << std::endl;
        auto read_res_indirect = this->block_manager_->read_block(indirect_block_id, indirect_block.data());
        if (read_res_indirect.is_err())
        {
          error_code = read_res_indirect.unwrap_error();
          goto err_ret;
        }
        auto indirect_block_p = reinterpret_cast<block_id_t *>(indirect_block.data());
        block_id = indirect_block_p[read_sz / block_size - inode_p->get_direct_block_num()];
      }
