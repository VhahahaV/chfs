...
#define LOGIC_2_RAW(i) (i - 1)
      ...
      -> ChfsResult<inode_id_t>
  {
         ...
         iter.next(bm->block_size()).unwrap(), count++)
    {
      ...
      if (free_idx)
      {
        ...
        auto inode_id = free_idx.value() + count * bm->block_size() * KBitsPerByte;
        // std::cout << "inode_id: " << inode_id << std::endl;

        std::vector<u8> buffer(bm->block_size());
        Inode newinode(type, bm->block_size());
        newinode.flush_to_buffer(buffer.data());
        bm->write_block(bid, buffer.data());
        // 2. Fill the inode table entry.
        set_table(inode_id, bid);

        // std::cout << "InodeManager::allocate_inode allocate_inode inode_id: " << RAW_2_LOGIC(inode_id) << std::endl;
  ...
  auto InodeManager::set_table(inode_id_t idx, block_id_t bid) -> ChfsNullResult
  {
    ...
    // whose index is `idx`.
    // UNIMPLEMENTED();
    // Calculate the table block index and offset.
    auto block_size = bm->block_size();
    // inode_per_block 512
    auto inode_per_block = block_size / sizeof(block_id_t);
    auto block_idx = idx / inode_per_block;
    auto offset = (idx % inode_per_block);
    // Read the table block.
    std::vector<u8> buffer(block_size);
    bm->read_block(block_idx, buffer.data());
    // Fill `bid` into the inode table entry.
    auto inode_table = reinterpret_cast<block_id_t *>(buffer.data());
    // std::cout << "offset : " << offset * sizeof(block_id_t *) << std::endl;
    inode_table[offset] = bid;
    // Write the table block back.
    bm->write_block(block_idx, buffer.data());
  ...
  auto InodeManager::get(inode_id_t id) -> ChfsResult<block_id_t>
  {
    ...
    std::vector<u8> buffer(block_size);
    bm->read_block(block_idx, buffer.data());
    // Get the block id of inode.
    auto inode_table = reinterpret_cast<block_id_t *>(buffer.data());
    res_block_id = inode_table[offset];
  ...
  {
    std::vector<u8> buffer(bm->block_size());
    std::cout << "get_type_attr inode_id: " << id << std::endl;
    auto res = this->read_inode(id, buffer);
  ...
  auto InodeManager::free_inode(inode_id_t id) -> ChfsNullResult
  {
    ...
    std::vector<u8> buffer(block_size);
    bm->read_block(block_idx, buffer.data());
    // Clear the inode table entry.
    auto inode_table = reinterpret_cast<block_id_t *>(buffer.data());
    inode_table[offset] = KInvalidBlockID;
    // Write the table block back.
    bm->write_block(block_idx, buffer.data());
    ...
    iter.next(block_idx * sizeof(block_id_t));
    auto data = iter.unsafe_get_value_ptr<u8>();
    auto bitmap = Bitmap(data, bm->block_size());
    bitmap.clear(offset);
