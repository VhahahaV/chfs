...
namespace chfs
{
      ...
      : file_name_(file), block_cnt(block_cnt), in_memory(false)
  {
    ...
    // std::cout << "you have implemented the log" << std::endl;
    // UNIMPLEMENTED();
    this->fd = open(file.c_str(), O_RDWR | O_CREAT, S_IRUSR | S_IWUSR);
    CHFS_ASSERT(this->fd != -1, "Failed to open the block manager file");
      ...
      -> ChfsNullResult
  {
    if (is_log_block)
    {
      ...
      for (int i = 0; i < this->block_sz; i++)
      {
        buffer[i] = data[i];
      }
      log_block_states.push_back(buffer);
      log_block_sizes.push_back(this->block_sz);
    }
    ...
    for (int i = 0; i < this->block_sz; i++)
    {
      this->block_data[this->block_sz * block_id + i] = data[i];
    }
    this->write_fail_cnt++;
    return KNullOk;
  }
      ...
      -> ChfsNullResult
  {
    if (is_log_block)
    {
      ...
      log_block_states.push_back(buffer);
      log_block_sizes.push_back(len);
    }
    // std::cout << "start write_partial_block" << std::endl;
    if (this->maybe_failed && block_id < this->block_cnt)
    {
      if (this->write_fail_cnt >= 3)
      {
    ...
    // strncpy((char *)this->block_data + this->block_sz * block_id + offset, (char *)data, len);
    for (int i = 0; i < len; i++)
    {
      this->block_data[this->block_sz * block_id + offset + i] = data[i];
    }
  ...
  auto BlockManager::read_block(block_id_t block_id, u8 *data) -> ChfsNullResult
  {
    ...
    for (int i = 0; i < this->block_sz; i++)
    {
      data[i] = this->block_data[this->block_sz * block_id + i];
    }
    return KNullOk;
  }
