...
namespace chfs
{
  ...
  auto BlockAllocator::allocate() -> ChfsResult<block_id_t>
  {
    ...
    for (uint i = 0; i < this->bitmap_block_cnt; i++)
    {
      ...
      if (res)
      {
        ...
        // Set the free bit we found to 1 in the bitmap.
        Bitmap(buffer.data(), bm->block_size()).set(*res);
        // Flush the changed bitmap block back to the block manager.
        bm->write_block(i + this->bitmap_block_id, buffer.data());
        // Calculate the value of `retval`.
  ...
  auto BlockAllocator::deallocate(block_id_t block_id) -> ChfsNullResult
  {
    ...
    auto block_id_in_bitmap = block_id / (bm->block_size() * KBitsPerByte);

    auto bitmap = Bitmap(buffer.data(), bm->block_size());
    bm->read_block(block_id_in_bitmap + this->bitmap_block_id, buffer.data());
    if (!bitmap.check(block_idx))
    {
      std::cout << "!bitmap.check(block_idx)" << std::endl;
      return ChfsNullResult(ErrorType::INVALID_ARG);
    }

    // 在block_idx为15时出错，clear无效而且而且write_block会将整个block写入
    bitmap.clear(block_idx);

    bm->write_block(block_id_in_bitmap + this->bitmap_block_id, buffer.data());
